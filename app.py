...
# Elles permettent aussi d'éviter l'overload
# de requêtes d'attaque et/ou de mouvement.
...
def index():
    """
    Page de jeu solo. On déclare game en global pour se
    servir de la même variable que celle déclarée avant.
    Si l'on passe du multi au single, on regarde si
    il ya une partie sauvegardée, sinon on en crée une nouvelle.
    Nous imposons un nom de session single par défaut.
    Ce n'est pas forcément indispensable.
    Les arguments passés dans le render servent
    ensuite dans le html via Jinja2.
    On passe la carte, sa taille, les stats du joueur
    et le niveau de la partie.
    """
    global game
    if game.mode == "Multi":
        try:
...
def login():
    """
    Page de login dans le mode multi.
    On rentre notre pseudo dans un form du html
    qui le transmet à la fonction login
    La session prend alors pour nom celui rentré.
    C'est ce qui permet ensuite d'identifier
    le joueur qui lance une action.
...
def multi():
    """
    Page de jeu multijoueur. On prend cette fois en compte
    le cas du passage du mode single au multi.
    Si c'est le cas, on lance une nouvelle partie et on renomme
    la session avec le pseudo choisi avant.
    Si un joueur est déjà présent dans la partie, on ajoute
    le nouveau et on actualise les stats des joueurs.
    """
    global game
    if game.mode == 'Single':
        game = Game()
    ...
    maping = game.getMap()
    return render_template("index.html", mapdata=maping,
    n_row=len(maping), n_col=len(maping[0]),
    golds=" ".join(str(player.golds) for player in game.players),
    HP=" ".join(str(player.health_points) for player in game.players),
    level=game.level,
    username=" ".join(player.name for player in game.players))


# Il y a à partir d'ici les fonctions qui se lancent
# seulement sur requête socket provenant du JavaScript.
...
def on_move_msg(json, methods=["GET", "POST"]):
    ...
    On construit cette fois une liste data qui
    contient les informations de position des joueurs,
    la carte mise à jour et les stats des joueurs
    mises à jour selon où ils ont marché.
    Cette liste est ensuite transmise au
    JavaScript via un socket.emit
    """
    dx = json['dx']
    dy = json['dy']
...
def door():
    """
    Requête door qui signifie que le joueur a
    amassé assez d'or pour révéler la porte secrète.
    On vérifie qu'elle n'est pas déjà révélée et
    on met à jour la carte avec la position de la porte.
...
def quit():
    ...
    On effectue ici une sauvegarde de game via
    le module pickle qui serialise l'objet
...
def monster_attack():
    """
    Requête d'attaque des monstres.
    On rencontre ici la même problématique qu'avec monster_move.
    On impose donc que les monstres attaquent toutes
    les 1.5 secondes au minimum.
...
def player_attack():
    """
    Requête d'attaque des joueurs.
    On identifie d'abord quel joueur a lancé l'attaque,
    ensuite l'attaque ne réussit que si le joueur est face à
    un autre joueur ou face à un monstre.
    Si le joueur/monstre attaqué voit des HP tomber à 0,
    on le fait disparaître de la partie et on
